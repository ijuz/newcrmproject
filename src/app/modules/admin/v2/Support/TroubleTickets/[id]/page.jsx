...
const FollowUpDetails = () => {
  ...
  const followUpId = "someId"; // Replace with the actual follow-up ID or dynamic logic

  useEffect(() => {
    const fetchFollowUp = async () => {
      try {
        const response = await axiosInstance.get(`v3/api/followups/${followUpId}`);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await axiosInstance.put(`v3/api/followups/${followUpId}`, updatedFollowUpData);
